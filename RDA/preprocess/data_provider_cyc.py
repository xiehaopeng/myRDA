...
from .data_list_cyc import ImageList_cyc
...
def load_images(images_file_path, batch_size, resize_size=256, is_train=True, crop_size=224, split_noisy=False, drop_last=True, is_eval=False):
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225])
    if is_train:    # 训练情况，使用含噪标签
        ...
        transformer = transforms.Compose([
            ...
            transforms.ToTensor(),
            normalize])
        if is_eval:     # 在验证情况下，如果images_file_path有三列(含噪声)，则用第三列(真实标签)为lable
            images = ImageList_cyc(open(images_file_path).readlines(), transform=transformer)
        else:           # 测试情况，还需要用含噪标签
            images = ImageList(open(images_file_path).readlines(), transform=transformer)
