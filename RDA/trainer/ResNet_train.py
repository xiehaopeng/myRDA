...
def evaluate(model_instance, input_loader):
    ...
    model_instance.set_train(False)     # 验证阶段设置为False
...
def train(model_instance, train_source_loader, test_target_loader, group_ratios, max_iter, optimizer, lr_scheduler, eval_interval):
    ...
    while True:
        for datas_clean in tqdm.tqdm(
            ...
            inputs_source, labels_source, _ = datas_clean       # 拿出一批source的数据与标签
            ...
            # 阶段性验证当前模型的准确率，间隔时长为eval_interval
    ...
    net_state_dict = model_instance.c_net.state_dict()

    return [loss, result], net_state_dict   # 返回训练结果与网络模型参数

# 一批训练数据放入网络中，得到total_loss，然后backward更新网络参数，并返回当前loss
...
if __name__ == '__main__':
    ...
    to_dump, net_state_dict = train(model_instance, train_source_loader, test_target_loader, group_ratios, max_iter=20000, optimizer=optimizer, lr_scheduler=lr_scheduler, eval_interval=1000)
