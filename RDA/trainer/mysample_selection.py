...
把带噪数据集分成易学样本(干净样本)、难学样本(feature noisy)、错误样本(ood/label noisy)
前两者是对网络训练有效的good样本，错误样本是bad样本
"""
...
if __name__ == '__main__':
    ...
    save_clean_file = source_file.split('.t')[0] + '_good_pred.txt'     # 分类器预测的good样本
    save_noisy_file = source_file.split('.t')[0] + '_bad_pred.txt'    # 分类器预测的bad样本
    nr = args.noisy_rate        # 原始数据集噪声比例

    # 检查获得数据集图片中每个样本种类 易学样本(clean样本)、难学样本(feature noisy)、错误样本(ood/label noisy)
    # is_easy, is_hard, is
    ...
    print("训练总轮数 Epoch {} vs 挑干净样本的正确率 Acc_clean {} vs 挑噪声样本的正确率 Acc_noisy {} vs 样本筛选率 cr {} vs 样本噪声比 nr {}".format(epoch,acc_clean,acc_noisy,cr,nr))
